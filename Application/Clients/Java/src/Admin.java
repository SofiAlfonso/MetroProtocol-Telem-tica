...
public class Admin extends JFrame {
    ...
    private final JTextArea responseArea = new JTextArea(3, 30);
    private final JScrollPane responseScroll = new JScrollPane(responseArea);
    ...
    public Admin() {
        ...
        response.setBorder(BorderFactory.createTitledBorder("Respuestas del Servidor"));
        responseArea.setEditable(false);
        responseArea.setFont(new Font(Font.MONOSPACED, Font.PLAIN, 12));
        response.add(responseScroll, BorderLayout.CENTER);
        center.add(response);
    ...
    private void sendCommand(String command) {
        ...
        responseArea.append("→ Enviando: " + command + "\n");
        ...
        try {
            ...
            responseArea.append("✗ Error de envío: " + ex.getMessage() + "\n");
            responseArea.setCaretPosition(responseArea.getDocument().getLength());
        }
    }
    ...
    private void processPayload(String payload) {
        ...
        if (trimmed.equals("OK")) {
            SwingUtilities.invokeLater(() -> {
                responseArea.append("✓ Comando ejecutado correctamente\n");
                responseArea.setCaretPosition(responseArea.getDocument().getLength());
            });
            return;
        }
        if (trimmed.startsWith("ERROR")) {
            SwingUtilities.invokeLater(() -> {
                responseArea.append("✗ Error: " + trimmed + "\n");
                responseArea.setCaretPosition(responseArea.getDocument().getLength());
            });
            return;
        }
